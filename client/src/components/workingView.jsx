...
const WorkingsView=()=>{
  ...
  const fetchWorkingData = async () => {
    ...
    const result = await fetch(`${process.env.REACT_APP_WORKINGSBYPERIOD_API_URL}/${selectedPeriod}/${id}`).then(
      (res) => res.json()
    )
   if(result){
 ...
 const fetchPeriod = async()=>{
  const periods = await fetch(process.env.REACT_APP_FETCH_PERIOD_URL)
                  .then(res=>res.json())
      ...
      <div className="container">
       {/* <h1>File to Table</h1> */}
        ...
        >
          ...
          {period&&period.map((period, index) => (
                        ...
                        <>
                          {workings.map((working, index) => (
                            ...
                            <tr key={index}>
                              ...
                              <td>{index + 1}</td>
                              <td>{working.user&&working.user.identityNumber}</td>
                              <td>{working.user&&working.user.name} {working.user&&working.user.surname}</td>
                              ...
                              <td>{working.fiili}</td>
                              <td>{working.fiili>working.activeWorkingTime?(working.fiili-working.activeWorkingTime):0}</td>
                              <td>{working.aksam}</td>
                              <td></td>
                              <td></td>
                              {
